...
public class ChatInfoController {
    ...
    @GetMapping("/selected-chat-new-message/{ciNum}")
    public int selectedChatNewMessage(@AuthenticationPrincipal UserInfoVO user, @PathVariable int ciNum) {
    	if(user != null) {
    		return chatInfoService.selectedChatNewMessage(user.getUiNum(), ciNum);
    	}
    	return 0;
    }
}